...
#[post("/receive")]
pub async fn receive_wallet(req: web::Json<ReceiveRequest>) -> impl Responder {
    // Basic check → address valid base58 & 32 bytes long
    if !bs58::decode(&req.public_address).into_vec().map(|v| v.len() == 32).unwrap_or(false) {
        return HttpResponse::BadRequest().json(serde_json::json!({
            "status": "error",
            "message": "Invalid public_address (must be base58-encoded 32 bytes)"
        }));
    }

    match make_qr_svg_base64(&req.public_address) {
        Ok(qr_code_svg_base64) => HttpResponse::Ok().json(ReceiveResponse {
