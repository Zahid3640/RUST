...
async fn import_wallet(req: web::Json<ImportRequest>) -> impl Responder {
    ...
    let payload = if let Some(seed_phrase) = seed_phrase_opt {
        ...
        json!({ "seed_phrase": null, "private_key": private_key_b58 })
    };
    ...
    HttpResponse::Ok().json(json!({"status":"success","public_key": public_key_b58}))
}

// ---------- UNLOCK ----------
#[post("/unlock_wallet")]
async fn unlock_wallet(req: web::Json<UnlockRequest>) -> impl Responder {
    let stored = load_wallet().unwrap();
    let key = derive_key(&req.password, &stored.kdf).unwrap();
    if let Err(_) = decrypt_payload(&key, &stored.crypto) {
        return HttpResponse::Unauthorized().json(json!({"status":"error","message":"Invalid password"}));
    }
    HttpResponse::Ok().json(json!({"status":"success","public_key": stored.public_key}))
}
